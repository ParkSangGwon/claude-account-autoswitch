...
Inicia sesión con dos o más cuentas de Claude y apunta Claude Code a `http://127.0.0.1:10912`.<br>
...
2. **Apunta Claude Code al proxy.** Una sola línea, con un botón Copiar bajo Ajustes → Proxy:
   ```sh
...
Claude Code conserva su propio inicio de sesión.<br>
...
- La app ejecuta un listener HTTP/1.1 en `127.0.0.1` (SwiftNIO).
- Las peticiones a cualquier ruta que no sea su propio plano de control se reenvían a `https://api.anthropic.com` con la cabecera `Authorization` de la cuenta elegida en lugar de la del cliente.
