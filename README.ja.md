...
- **作業を始めた時刻ではなく、時計どおりに回るウィンドウ。**
  - Claude は最初のリクエストで5時間ウィンドウを開始するため、16:30 に触れたウィンドウは 21:30 まで続き、一日に回せる本数がその分減ります。
  - ポップオーバーのリセットタイムラインの下にある *5時間ウィンドウを開いたままにする* をオンにすると、各アカウントのウィンドウがリセットされ次第、次のウィンドウを開きます。
  - リクエストはあなたのアカウントで送られるため、自分でオンにするまではオフのままで、スリープしていた Mac は眠っている間に過ぎたリセットを復帰から1分以内に開きます。
- **実際のケースに対応するローテーション。**
  ...
  - その後の拒否応答が閉じたウィンドウと再び開く時刻を名指しするので、Claude Code はエラーで止まらず、待ってから自分で作業を再開します。
