...
2 つ以上の Claude アカウントでサインインし、Claude Code の前にプロキシを置きます。<br>
...
Claude Code が読む上限は 1 アカウントのものではなくローテーション全体のものなので、別のアカウントに余裕がある間は上限の案内が出ません。<br>
...
2. **Claude Code の前にプロキシを置く。** 設定 → プロキシに、コピーボタン付きでこの 1 行が表示されています:
   ```sh
...
セットアップはこれだけです。<br>
Claude Code のログインはそのままで、引き続き `api.anthropic.com` と通信するため、リモートコントロール・管理された設定・組織ポリシーもそのまま動作します。<br>
プロキシは送信時にトークンを差し替えるだけで、リクエストのほかの部分には手を触れません。

## 証明書

プロキシが `api.anthropic.com` の前に立つということは、そのホストの TLS を終端するということであり、Claude Code が受け入れる証明書が必要だということです。<br>
アプリはこの Mac 上に認証局を作り、設定ファイルの `NODE_EXTRA_CA_CERTS` 変数を通じて Claude Code だけをそれに向けます。<br>
システムキーチェーンには**追加しません**。ブラウザも、ほかのアプリも、ほかのツールもこれを信頼せず、既定では何もこれに向けられていません。<br>
Claude Code がこれを信頼している間、プロキシはそのプロセスの Claude API トラフィックを復号して再暗号化します — それがトークンを差し替える仕組みであり、`ANTHROPIC_BASE_URL` を使っていた以前のリリースも同じリクエストを平文で見ていました。<br>
CA の**秘密鍵**を持つものは Claude Code が受け入れる証明書を発行できるため、その鍵はディスクに書きません。更新はチェーン全体を作り直す方式で、保存される秘密はホスト 1 つ分の leaf 鍵だけです。<br>
アプリのフォルダを削除すれば信頼も消え、システムキーチェーンには何も残りません。
...
- **全アカウントをひと目で。**
  ...
  - 行メニュー: 現在のアカウントにする、有効にする、しばらくスキップ、優先度、削除。
- **次のリクエストの行き先と、その理由。**
...
- **実際のケースに対応するローテーション。**
  ...
  - ウィンドウを名指ししない 429 はリクエストだけを次のアカウントに回し、アカウント自体はローテーションに残します。繰り返されたときだけ退避します。
  - 期限切れトークンは 1 回だけ更新して再試行します。
  - 403 と 5xx はフェイルオーバーします。
  - 全アカウントが枯渇したときは、失敗する代わりに設定した時間だけリクエストを保留できます。
  - 再起動するとローテーションは中断した場所から続きます。すでに枯渇したアカウントに最初のリクエストが行くことはありません。
- **セッション。**
  - 各 Claude Code セッションは週次バケットごとに同じアカウントにとどまります。
  - オプションの均等分配で、新しいセッションを最も空いているアカウントに割り当てます。
- **おかしいときは、そう言います。**
  - 読み取れない設定ファイルは決して上書きせず、どのキーを直せばよいかをエラーが伝えます。
  - ポートを占有しているプログラム名を示して空きポートを提案し、プロキシに誰も話しかけていなければそう知らせます。
- **どこからでも切り替え。**
...
- アプリは `127.0.0.1` でプロキシ（SwiftNIO）を動かし、Claude Code は `HTTPS_PROXY` 経由でそこに届きます。
- `CONNECT api.anthropic.com:443` はアプリ自身が終端し、クライアントの `Authorization` を選ばれたアカウントのものに置き換えて上流へ転送します。ほかのホストは手を触れずトンネルします。
...
それ以外のホストへのトラフィックは復号されずにプロキシを通過します。<br>
