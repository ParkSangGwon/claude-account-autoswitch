...
它读到的限额是整个轮换的，而不是某一个账户的，所以只要还有账户有余量，就不会弹出限额提示。<br>
...
- **每个账户一目了然。**
  ...
  - 行菜单：设为当前、启用、暂时跳过、优先级、移除。
- **下一个请求去哪里，以及为什么。**
  - 原账户的原因、更高的优先级，或“仍使用 ted”。
- **全部账户汇总和重置时间线。**
  - 按层级加权的汇总，只统计仍能使用该窗口的账户。
  - 每一次即将到来的窗口重置，能让账户重新加入轮换的标有 `↑`。
- **按时钟推进的窗口，而不是从你坐下来时才开始。**
  - Claude 在你发出第一个请求时才开启 5 小时窗口，所以 16:30 才动到的窗口会一直算到 21:30，一天能跑的窗口数因此变少。
  - 打开弹出面板中重置时间线下方的 *保持 5 小时窗口开启*，每个账户的窗口一重置就会立刻开启下一个。
  - 请求会以你的账户发出，因此在你主动开启前一直是关闭的；休眠过的 Mac 会在唤醒后一分钟内补开睡眠期间错过的重置。
- **能应对真实情况的轮换。**
  - 指明窗口已关闭的 429 会按其 retry-after 让账户受限。
  - 未指明窗口的 429 只把请求转给下一个账户，账户仍留在轮换中；只有反复出现时才让位。
  - 过期令牌刷新一次后重试。
  - 403 和 5xx 故障转移。
  - 所有账户耗尽时，请求可以挂起一段可配置的时间，而不是直接失败。
  - 随后的拒绝响应会指明关闭的窗口以及它重新开放的时间，因此 Claude Code 会等待并自行接着做下去，而不是停在错误上。
  - 重启后轮换从上次停下的地方继续，不会把第一个请求送到已经耗尽的账户。
- **会话。**
  - 每个 Claude Code 会话按每周配额桶固定在自己的账户上。
  - 可选的均匀分配会把新会话分给负载最低的账户。
- **出问题时会直说。**
  - 读不了的配置文件绝不会被覆盖，错误会指出该去修哪个键。
  - 会说出占用端口的程序并提供空闲端口；没有人经过代理时也会提示。
- **随时随地切换。**
