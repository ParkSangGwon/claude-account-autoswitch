...
- An origin-form request is served directly. `GET /_autoswitch/health` answers `{"ok":true,"version":"…","port":10912,"accounts":2,"startedAt":"…"}`, and `GET /_autoswitch/session/<id>` answers `{"session":"…","account":"<label>","accountID":"…"}` for the account that last served the Claude Code session whose `x-claude-code-session-id` is `<id>`, or 404 for a session the app has not seen. These two are the only endpoints the app itself serves; anything else in that form is a client still pointed here with `ANTHROPIC_BASE_URL`, which is served as before and counted so the app can say that session has Remote Control switched off. An absolute-form request is refused with 501.
